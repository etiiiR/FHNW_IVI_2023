...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Label("Wählen Sie den Uber Type", style={'font-weight': 'bold'}),
        dcc.Dropdown(
            id='uber-type-dropdown',
            options=[{'label': 'Alle', 'value': 'Alle'}] + [{'label': str(i), 'value': i} for i in uber_df['Uber Type'].unique()],
            value='Alle'
        ),
        html.Label("Fahrpreisbereich:", style={'font-weight': 'bold', 'margin-top': '10px'}),
        dcc.RangeSlider(
            id='fare-range-slider',
            min=uber_df['Fare$'].min(),
            max=uber_df['Fare$'].max(),
            step=100,
            marks={i: f'${i}' for i in range(int(uber_df['Fare$'].min()), int(uber_df['Fare$'].max()) + 1, 300)},
            value=[uber_df['Fare$'].min(), uber_df['Fare$'].max()]
        ),
        html.Label("Entfernungsbereich:", style={'font-weight': 'bold', 'margin-top': '10px'}),
        dcc.RangeSlider(
            id='distance-range-slider',
            min=uber_df['Journey Distance(m)'].min(),
            max=uber_df['Journey Distance(m)'].max(),
            step=1,
            marks={i: f'{i}m' for i in range(0, int(uber_df['Journey Distance(m)'].max()) + 1, 10000)},
        ...
        dcc.DatePickerRange(
            ...
            style={
                'margin-top': '20px', 
                'transform': 'scale(1.0)',  # Skalierung des Elements
                'transform-origin': 'left center',
                'width': '100%',  # Ursprungspunkt der Skalierung
            }
              # Adjust this factor to your data
        ),
    ], style={'grid-area': 'sidebar', 'padding': '10px'}),

    # Hauptinhalt
    html.Div([
        html.H1("Deine Uber Fahrten", style={'textAlign': 'center', 'font-size': '30px', 'font-weight': 'bold'}),
        dcc.Graph(id='map-plot', style={'height': '60vh'}),
         # Zusätzliche Visualisierungen
        html.Div([
            dcc.Graph(
                id='fare_histogram',
                figure=px.histogram(uber_df, x='Fare$', title='Verteilung der Fahrpreise')
            ),
            dcc.Graph(
                id='distance_scatter',
                figure=px.scatter(uber_df, x='Journey Distance(m)', y='Fare$', title='Fahrpreis vs. Reiseentfernung'),
            ),
